...
class VamtamEnqueues {
	...
	public static function admin_scripts() {
		...
		wp_localize_script(
			'vamtam-admin', 'VAMTAM_ADMIN', array(
				...
				'home_url'   => home_url(),
				'ownStoreTxt' => esc_html__( 'Enter your purchase code from vamtam.com to receive theme updates and support.', 'skole' ),
				'elementsTxt' => esc_html__( 'Enter your Envato Elements License Code.', 'skole' ),
				'tfPcTxt' => esc_html__( 'Enter your purchase code from ThemeForest to receive theme updates and support.', 'skole' ),
				'unRegOwnStoreTxt' => esc_html__( 'You are about to unregister your vamtam.com purchase code. Are you sure you want to continue?', 'skole' ),
				'unRegPcTxt' => esc_html__( 'You are about to unregister your purchase code. Are you sure you want to continue?', 'skole' ),
				'unRegTokenTxt' => esc_html__( 'You are about to unregister your Envato Elements License Code. Are you sure you want to continue?', 'skole' ),
				'unRegInvalidTokenTxt' => esc_html__( 'There seems to be an issue with your current license code registration, likely due to site migration. If you proceed, the theme will be unregistered at your end but the license code will remain bound to the URL it was registered at, until you unregister it from there. If you\'d like to re-register this as an Envato Elements purchase, you can always generate a new license code. Are you sure you want to continue?', 'skole' ),
				'invalidOwnStoreTxt' => sprintf(
					/* translators: %s: link to the vamtam.com Order Details page. */
					esc_html__( 'This does not look like a vamtam.com purchase code. It should look like VAMTAM-XXXXX-XXXXX-XXXXX-XXXXX-XXXXX-XXXXX. Please check your %s.', 'skole' ),
					'<a href="https://vamtam.com/my-account/orders/" target="_blank">' . esc_html__( 'Order Details on vamtam.com', 'skole' ) . '</a>'
				),
				'invalidMarketTxt' => sprintf(
					/* translators: %s: link to the Envato "Where is my purchase code" help article. */
					esc_html__( 'This does not look like an Envato Market purchase code. It should look like xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx. %s', 'skole' ),
					'<a href="https://help.market.envato.com/hc/en-us/articles/202822600-Where-Is-My-Purchase-Code-" target="_blank">' . esc_html__( 'Where can I find my purchase code?', 'skole' ) . '</a>'
				),
				'invalidElementsTxt' => sprintf(
					/* translators: %s: link to the "where to get your Envato Elements license code" help article. */
					esc_html__( 'This does not look like an Envato Elements license code. It should look like AVUF6MZXN7. %s', 'skole' ),
					'<a href="https://elementor.support.vamtam.com/support/solutions/articles/252289-cannot-validate-purchase-key-or-token" target="_blank">' . esc_html__( 'Where to get the License Code from', 'skole' ) . '</a>'
				),
		...
		if ( $pagenow === 'update-core.php' ) {
			...
			<script type="text/javascript">
				document.addEventListener( 'DOMContentLoaded', function() {
					...
					if ( ! $valid_pc ) {
						$update_notice  = '<br id="vamtam-notice-br" />' . sprintf(
							/* translators: %1$s-%2$s: registration page link, %3$s-%4$s: purchase link. */
							__( 'You need a valid Purchase Code to use the Automatic Updates feature. Register your Purchase Code %1$shere%2$s if you already have one, or %3$sget one here%4$s.', 'skole' ),
	...
	public static function print_theme_options() {
		...
		if ( class_exists( 'VamtamElementorBridge' ) && VamtamElementorBridge::is_elementor_active() ) {
			...
			// Request the Google fallback fonts with font-display: swap (the `display` query
			// arg) so text paints immediately instead of blocking on the webfont.
			$fallback_fonts_url = include( VAMTAM_THEME_DIR . 'samples/elementor-styles-fallback-fonts.php' );
			if ( $fallback_fonts_url && false === strpos( $fallback_fonts_url, 'display=' ) ) {
				$fallback_fonts_url .= ( false === strpos( $fallback_fonts_url, '?' ) ? '?' : '&' ) . 'display=swap';
			}
			wp_enqueue_style( 'vamtam-elementor-fallback-fonts', $fallback_fonts_url, [], VamtamFramework::get_version() );
	...
	/**
	 * When editing a post, cap the editor canvas to the same content width the
	 * front-end uses (the boxed width of the section wrapping the theme-post-content
	 * widget in the active Elementor Single template), so the editor isn't wider
	 * than the real post. Set on .editor-styles-wrapper (a class selector, always an
	 * ancestor of .wp-block) so it beats the body-level default from print_theme_options().
	 */
	public static function get_post_editor_width_css() {
		...
		$post_id = get_the_ID();
		$width   = ( $post_id && class_exists( 'VamtamElementorBridge' ) )
			? VamtamElementorBridge::resolve_single_post_content_width( $post_id )
			: null;

		if ( ! $width ) {
			// ponytail: falls back to blog/single.less's value when the Single template
			// can't be resolved safely (unusual conditions, non-px width, etc.).
			$width = 1000;
		}

		return '.editor-styles-wrapper{--vamtam-site-max-width:' . (int) $width . 'px;}';
	}
	...
	public static function block_editor_variables() {
		...
		$css = self::get_block_editor_variables_css() . self::get_post_editor_width_css();
