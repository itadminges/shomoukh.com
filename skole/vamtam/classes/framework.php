...
class VamtamFramework {
	...
	public static function get_signature() {
		...
		if ( empty( $signature ) ) {
			...
			if ( '' === $nonce_key && '' === $nonce_salt ) {
				wp_salt('nonce'); // create fallback values stored in the database
			...
			<div class="vamtam-ts-notice">
				<div class="vamtam-notice vamtam-license-notice notice">
					...
					<div class="vamtam-notice-content">
						<h3>Attention:</h3>
						<?php if ( $is_token ) : ?>
							<p>This Envato Elements license code has been activated on too many websites. License codes are activated per-domain (1 License Code = 1 Domain).</p>
							...
							<p>You can generate a new license code for use in the new domain by following
							<?php
						...
						<?php endif; ?>
						<?php // the AJAX response has no page URL to link back to ?>
						<?php if ( ! wp_doing_ajax() ) : ?>
							<p>
								<a class="button" href="<?php echo esc_url( wp_nonce_url( add_query_arg( 'vamtam-recheck-license', '1' ), 'vamtam-recheck-license' ) ); ?>">
									<?php esc_html_e( 'Re-check now', 'skole' ); ?>
								</a>
							</p>
						<?php endif; ?>
					</div>
		...
		?>
			<div class="vamtam-ts-notice">
				<div class="vamtam-notice vamtam-license-notice notice">
					...
					<div class="vamtam-notice-content">
						<h3>Attention:</h3>
						<p>This Envato Elements license code is already in use on a different domain. To connect the license code to a new domain, please disconnect it from the existing one first.</p>
						<p>Alternatively, you can generate a new license code for use in the new domain by following
						<?php
							echo wp_kses( sprintf( __( ' <a href="%s" target="_blank"> this link</a>', 'skole' ), esc_url( 'https://api.extensions.envato.com/extensions/begin_activation'
							. '?extension_id=' . md5( get_site_url() )
							. '&extension_type=envato-wordpress'
	...
	public static function token_unregistered() {
		echo '<span id="success">';
		esc_html_e('Unregistered License Code.', 'skole');
		echo '</span>';
	}

	public static function token_unregister_fail() {
		echo '<span id="fail">';
		esc_html_e('Failed to unregister license code. Please try again later. If this issue persists please contact support.', 'skole');
		echo '</span>';
	}
